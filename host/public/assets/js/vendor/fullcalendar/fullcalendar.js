...
var TimeGrid = Grid.extend({
	...
	slatTops: null, // an array of top positions, relative to the container. last order holds bottom of last slot
	...
	computeRowCoords: function() {
		...
		for (i = 0; i < this.rowCnt; i++) {
			...
			if (i > 0) {
				orders[i - 1].bottom = order.top;
			}
			orders.push(order);
		}
		order.bottom = order.top + this.computeTimeTop(this.computeSnapTime(i));
	...
	// Includes the the bottom of the last slat as the last order in the array.
	...
	function expandEvent(abstractEvent, _rangeStart, _rangeEnd) {
		...
		if (abstractEvent) {
			if (abstractEvent._recurring) {
				...
				events.push(abstractEvent); // return the original event. will be a one-order array
			}
		}
