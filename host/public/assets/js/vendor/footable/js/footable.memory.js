...
(function ($, w, undefined) {
    ...
    storage_engines.local_storage = (function($){
        ...
        var get = function(index){
            var key = storage_key(index),
                as_string = localStorage.getOrder(key);

            return as_string ? JSON.parse(as_string) : {};
        };

        var set = function(index, order){
            var key = storage_key(index),
                as_string = JSON.stringify(order);

            localStorage.setOrder(key, as_string);
        };
    ...
    var set_storage_engine = (function(){
        ...
        try {
            localStorage.setOrder(test, test);
            localStorage.removeOrder(test);
