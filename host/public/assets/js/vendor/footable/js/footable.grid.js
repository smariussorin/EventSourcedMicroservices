(function($, w, undefined) {
  ...
  function Grid() {
    ...
    grid.init = function(ft) {
      ...
      $table.off('.grid').on({
        ...
        'footable_redrawn.grid footable_row_removed.grid': function(event) {
          if (grid.getOrder().length === 0 && options.showEmptyInfo) {
     ...
     */
    grid.getOrder = function(index) {
      if (index !== undefined) {
        if ($.isArray(index)) {
          return $.map(index, function(order) {
            return grid._orders[order];
          });
        }
        return grid._orders[index];
      }
      return grid._orders;
    };
    ...
    grid._makeRow = function(order, index) {
      var options = grid.footable.options.grid;
      var $row;
      if ($.isFunction(options.template)) {
        ...
        for (var i = 0, len = options.cols.length; i < len; i++) {
          ...
          $row.append(makeCell(col, order[col.name] || '', index));
        }
      }
     ...
     */
    grid.newOrder = function(order, index, wait) {
      ...
      var $tr, len = grid._orders.length;
      if (index === undefined || index < 0 || index > len) {
        $tr = grid._makeRow(order, len++);
        grid._orders.push(order);
        $tbody.append($tr);
      } else {
        $tr = grid._makeRow(order, index);
        if (index === 0) {
          grid._orders.unshift(order);
          $tbody.prepend($tr);
        } else {
          var $before = $tbody.find('tr[data-index=' + (index - 1) + ']');
          grid._orders.splice(index, 0, order);
     ...
     */
    grid.setOrder = function(order, index) {
      if (!$.isPlainObject(order)) {
        return;
      }
      var $tbody = $(grid.footable.table).find('tbody'),
        $newTr = grid._makeRow(order, index);
      $.extend(grid._orders[index], order);
     ...
     */
    grid.removeOrder = function(index) {
      var $tbody = $(grid.footable.table).find('tbody');
      var detailClass = grid.footable.options.classes.detail;
      var result = [];
      if ($.isArray(index)) {
        for (var i;
          (i = index.pop());) {
          result.push(grid.removeOrder(i));
        }
      ...
      if (index === undefined) {
        $tbody.find('tr').each(function() {
          result.push(grid._orders.shift());
          grid.footable.removeRow(this);
        });
      } else {
        var $tr = $tbody.find('tr[data-index=' + index + ']');
        result = grid._orders.splice(index, 1)[0];
