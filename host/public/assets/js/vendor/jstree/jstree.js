 ...
 */
	...
	_node.setAttribute('role', 'treeorder');
	...
	$.jstree.defaults.core = {
		 ...
		 * data configuration
		 *
		 * If left as `false` the HTML inside the jstree container element is used to populate the tree (that should be an unordered list with list orders).
		 *
	...
	$.jstree.core.prototype = {
		...
		init : function (el, options) {
			...
			this._data.core.original_container_html
				...
				.remove();
			this.element.html("<"+"ul class='jstree-container-ul jstree-children' role='group'><"+"li id='j"+this._id+"_loading' class='jstree-initial-node jstree-loading jstree-leaf jstree-last' role='tree-order'><i class='jstree-icon jstree-ocl'></i><"+"a class='jstree-anchor' href='#'><i class='jstree-icon jstree-themeicon-hidden'></i>" + this.get_string("Loading ...") + "</a></li></ul>");
		...
		load_node : function (obj, callback) {
			...
			if(obj.state.loaded) {
				...
				for(k = 0, l = obj.children_d.length; k < l; k++) {
					for(i = 0, j = obj.parents.length; i < j; i++) {
						this._model.data[obj.parents[i]].children_d = $.vakata.array_remove_order(this._model.data[obj.parents[i]].children_d, obj.children_d[k]);
					}
					if(this._model.data[obj.children_d[k]].state.selected) {
						c = true;
						this._data.core.selected = $.vakata.array_remove_order(this._data.core.selected, obj.children_d[k]);
					}
		...
		deselect_node : function (obj, supress_event, e) {
			...
			if(obj.state.selected) {
				...
				this._data.core.selected = $.vakata.array_remove_order(this._data.core.selected, obj.id);
				if(dom.length) {
		...
		refresh : function (skip_loading, forget_state) {
			...
			if(!skip_loading) {
				this.element.html("<"+"ul class='"+c+"' role='group'><"+"li class='jstree-initial-node jstree-loading jstree-leaf jstree-last' role='treeorder' id='j"+this._id+"_loading'><i class='jstree-icon jstree-ocl'></i><"+"a class='jstree-anchor' href='#'><i class='jstree-icon jstree-themeicon-hidden'></i>" + this.get_string("Loading ...") + "</a></li></ul>");
				this.element.attr('aria-activedescendant','j'+this._id+'_loading');
			}
		...
		move_node : function (obj, par, pos, callback, is_loaded, skip_redraw, origin) {
			...
			if(obj.parent === new_par.id) {
				...
				old_ins._model.data[old_par].children = $.vakata.array_remove_order(old_ins._model.data[old_par].children, obj.id);
	...
	// remove order from array
	$.vakata.array_remove_order = function(array, order) {
		var tmp = $.inArray(order, array);
	...
	$.jstree.plugins.checkbox = function (options, parent) {
							...
							while(p && p.id !== '#') {
								...
								if(c === j) {
									...
									if(p.state[ t ? 'selected' : 'checked' ]) {
										...
										this._data[ t ? 'core' : 'checkbox' ].selected = $.vakata.array_remove_order(this._data[ t ? 'core' : 'checkbox' ].selected, p.id);
										tmp = this.get_node(p, true);
										if(tmp && tmp.length) {
											tmp.attr('aria-selected', false).children('.jstree-anchor').removeClass(t ? 'jstree-clicked' : 'jstree-checked');
										}
		...
		this.uncheck_node = function (obj, e) {
			...
			if(obj.state.checked) {
				...
				this._data.checkbox.selected = $.vakata.array_remove_order(this._data.checkbox.selected, obj.id);
				if(dom.length) {
				...
				if(tmp && tmp.state.loaded) {
					for(k = 0, l = tmp.children_d.length; k < l; k++) {
						if(this._model.data[tmp.children_d[k]].state.checked) {
							c = true;
							this._data.checkbox.selected = $.vakata.array_remove_order(this._data.checkbox.selected, tmp.children_d[k]);
						}
					}
				}
			}
 ...
 */
	...
	$.jstree.defaults.contextmenu = {
		...
		show_at_node : true,
		/**
		 * an object of actions, or a function that accepts a node and a callback function and calls the callback function with an object of actions available for that node (you can also return the orders too).
		 *
		 * Each action consists of a key (a unique name) and a value which is an object with the following properties (only label and action are required):
		 *
		 * * `separator_before` - a boolean indicating if there should be a separator before this order
		 * * `separator_after` - a boolean indicating if there should be a separator after this order
		 * * `_disabled` - a boolean indicating if this action should be disabled
		 * * `label` - a string - the name of the action (could be a function returning a string)
		 * * `action` - a function to be executed if this order is chosen
		 * * `icon` - a string, can be a path to an icon or a className, if using an image that is in the current directory use a `./` prefix, otherwise it will be detected as a class
		 * * `shortcut` - keyCode which will trigger the action if the menu is open (for example `113` for rename, which equals F2)
		 * * `shortcut_label` - shortcut label (like for example `F2` for rename)
		 *
		 * @name $.jstree.defaults.contextmenu.orders
		 * @plugin contextmenu
		 */
		orders : function (o, cb) { // Could be an object directly
			return {
				"create" : {
	...
	$.jstree.plugins.contextmenu = function (options, parent) {
		 ...
		 * @param {Number} x the x-coordinate relative to the document to show the menu at
		 * @param {Number} y the y-coordinate relative to the document to show the menu at
		 * @param {Number} i the object of orders to show
		 * @plugin contextmenu
		 * @trigger show_contextmenu.jstree
		 * @private
		 */
	...
	(function ($) {
		...
		$.vakata.context = {
			...
			_execute : function (i) {
				i = vakata_context.orders[i];
				return i && (!i._disabled || ($.isFunction(i._disabled) && !i._disabled({ "order" : i, "reference" : vakata_context.reference, "element" : vakata_context.element }))) && i.action ? i.action.call(null, {
							"order"		: i,
							"reference"	: vakata_context.reference,
							"element"	: vakata_context.element,
							"position"	: {
			...
			_parse : function (o, is_callback) {
				...
				$.each(o, function (i, val) {
					if(!val) { return true; }
					vakata_context.orders.push(val);
					if(!sep && val.separator_before) {
						str += "<"+"li class='vakata-context-separator'><"+"a href='#' " + ($.vakata.context.settings.icons ? '' : 'style="margin-left:0px;"') + ">&#160;<"+"/a><"+"/li>";
					}
					sep = false;
					str += "<"+"li class='" + (val._class || "") + (val._disabled === true || ($.isFunction(val._disabled) && val._disabled({ "order" : val, "reference" : vakata_context.reference, "element" : vakata_context.element })) ? " vakata-contextmenu-disabled " : "") + "' "+(val.shortcut?" data-shortcut='"+val.shortcut+"' ":'')+">";
					str += "<"+"a href='#' rel='" + (vakata_context.orders.length - 1) + "'>";
					...
					str += ($.isFunction(val.label) ? val.label({ "order" : i, "reference" : vakata_context.reference, "element" : vakata_context.element }) : val.label) + (val.shortcut?' <span class="vakata-contextmenu-shortcut vakata-contextmenu-shortcut-'+val.shortcut+'">'+ (val.shortcut_label || '') +'</span>':'') + "<"+"/a>";
					if(val.submenu) {
			...
			show : function (reference, position, data) {
				...
				if(vakata_context.orders.length) {
 ...
 */
	...
	$.jstree.defaults.dnd = {
		...
		touch : true,
		/**
		 * controls whether orders can be dropped anywhere on the node, not just on the anchor, by default only the node anchor is a valid drop target. Works best with the wholerow plugin. If enabled on mobile depending on the interface it might be hard for the user to cancel the drop, since the whole tree container will be a valid drop target.
				...
				.on('before_open.jstree', $.proxy(function (e, data) {
						...
						if(r && r.length) {
							...
							if(this._data.search.som && this._data.search.res.length) {
								...
								s = $.vakata.array_remove_order($.vakata.array_unique(s),'#');
 ...
 */
	...
	(function ($, undefined) {
		$.vakata.storage = {
			...
			set : function (key, val) { return window.localStorage.setOrder(key, val); },
			get : function (key) { return window.localStorage.getOrder(key); },
			del : function (key) { return window.localStorage.removeOrder(key); }
		};
	}($));
